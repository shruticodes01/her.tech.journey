...
const separateFlightDetails = flights.split('+');
console.log(separateFlightDetails);

const newFlightTextArr = [];
const allFlightsTransformText = function () {
  for (let flight of separateFlightDetails) {
    flight = flight.replaceAll('_', ' ').replaceAll(':', 'h').trim().split(';');
    console.log(flight);

    const arrStr1 = `from ${flight[1].slice(0, 3).toUpperCase()}`;

    const arrStr2 = `to ${flight[2].slice(0, 3).toUpperCase()}`;
    const newStr =
      flight[1].replace(flight[1], arrStr1) +
      ' ' +
      flight[2].replace(flight[2], arrStr2);

    newFlightTextArr.push(
      `${flight[0].startsWith('Delayed') ? '🔴' : ''} ${flight[0]} ${newStr} (${
        flight[3]
      })`.padStart(50)
    );
  }
  console.log(newFlightTextArr);

  for (const text of newFlightTextArr) {
    console.log(text);
  }
};
allFlightsTransformText();
